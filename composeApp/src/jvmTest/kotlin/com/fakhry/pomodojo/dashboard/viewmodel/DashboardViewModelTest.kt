...
class DashboardViewModelTest {
    ...
    fun `initial state uses stored focus minutes`() = runTest {
        val initialPreferences = PomodoroPreferences(focusMinutes = 30)
    ...
    fun `timer updates when focus minutes change`() = runTest {
        val initialPreferences = PomodoroPreferences(focusMinutes = 25)
    ...
    private fun createRepository(initialPreferences: PomodoroPreferences): PreferencesRepository {
        val storage = FakePreferenceStorage(initialPreferences)
        return PreferencesRepository(storage, PreferenceCascadeResolver())
    }

    private class FakePreferenceStorage(initial: PomodoroPreferences) : PreferenceStorage {
